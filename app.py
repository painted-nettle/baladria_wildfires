...
def load_data():
    ...
    leaflets_df = pd.read_csv('leaflets.csv', sep='¦', encoding='latin-1')
...
earliest_delivery_date, latest_delivery_date, unique_org_types, unique_cities, unique_neighbourhoods, \
    min_age, max_age, min_income, max_income, min_house_price, max_house_price, max_cars, max_occupants, \
    dists_orgs_df, leaflets_df = load_data()
...
with st.sidebar:
    ...
    with st.expander(label='Analysis'):
        analysis_type = st.radio(label='Select analysis type',
                                 options=['None (raw data)', 'Demographics', 'Sentiment', 'Topics'])
...
try:
    ...
    st.markdown("Metrics below are based on your current filter settings.")
    ...
    if analysis_type == 'None (raw data)':
        ...
        with sub_tab_economic:
            ...
            st.markdown(
                "These heatmaps show the concentration of recipients. Brighter areas indicate more individuals.")
        ...
        with sub_tab_orgs:
            ...
            st.markdown("Which topics are most frequently used by which organizations (based on your filters)?")
            ...
            fig_topic_orgs = px.histogram(
                ...
                labels={'Name': 'Organization', 'dominant_topic': 'Topic ID'}
            )
